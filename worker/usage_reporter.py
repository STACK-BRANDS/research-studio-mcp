...
Behavioral contract (§4.2, "never break the bot"):
  ...
  - No threads. Flushes are synchronous only (buffer-full / explicit /
...
class UsageReporter:
    ...
    def __init__(self, system: str, repo: str, run_id: str = None):
        self.system = system
        self.repo = repo
    ...
    def spend(self, action, model=None, input_tokens=None, output_tokens=None,
        ...
        server derives cost from (model, input_tokens, output_tokens)."""
    ...
    def _record(self, action, kind, model=None, input_tokens=None,
                output_tokens=None, cost=None, quantity=None, unit=None,
                store_id=None, meta=None):
        if self._noop:
            return
        event = {
            "system": self.system,
    ...
    def _flush(self) -> None:
        ...
        if not self._buffer:
            return
        # Send in batches of at most 50 events per the wire contract. Every
        # batch is removed from the buffer whether or not the POST
        # succeeds -- there is no retry queue (bounded memory, no backoff
        # loop); a failed batch is logged and dropped, not re-buffered.
        while self._buffer:
            batch, self._buffer = self._buffer[:_MAX_EVENTS_PER_BATCH], self._buffer[_MAX_EVENTS_PER_BATCH:]
            self._post(batch)
